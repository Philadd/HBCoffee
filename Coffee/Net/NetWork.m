    ...
    if (ver != tempCountVer) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            for (int i = 0; i < count; i += maxTempCount) {
                ...
                sendCount++;
                if (dispatch_semaphore_wait(self.signal, DISPATCH_TIME_FOREVER) != 0) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            for (int i = (int)[self.yVals_Out count]; i < count; i += maxTempCount) {
                if (i + maxTempCount < count) {
                    [self inquireTempWithCount:maxTempCount startPosition:i];
                }else if (i != count){
                    [self inquireTempWithCount:count - i startPosition:i];
                }
                if (dispatch_semaphore_wait(self.signal, time) != 0) {//返回非0表示超时，返回0则正常
                    i -= maxTempCount;
                }
            }
        });
...
- (void)getTemp{
    ...
    dispatch_async(_queue, ^{
        ...
        [self.mySocket readDataWithTimeout:-1 tag:2];
    });
}
...
- (void)getTime{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [self performSelector:@selector(getTime) withObject:nil afterDelay:3.0f];
    });
}
    ...
    if (_recivedData68)
    {
        ...
        if (self.frame68Type == readReplyFrame) {
            if (self.msg68Type == fire) {
                ...
                if (tempBean > self.alertTemp && self.alertTemp > 0.f && _isAlert) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [self showTemperatureAlertAction];
                    });
                }
                
                //保存纯数字而不是ChartDataEntry（曲线数据）
...
//                });
            ...
            if (self.msg68Type == getTimerStatus) {
                if ([_recivedData68[6] unsignedIntegerValue] == 0) {
                    ...
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"isStartBake" object:nil userInfo:nil];
                    [_myTimer setFireDate:[NSDate distantFuture]];
                ...
                }else if ([_recivedData68[6] unsignedIntegerValue] == 2){
                    [self setDeviceTimerStatus:[_recivedData68[6] intValue]];
...
- (void)showTemperatureAlertAction{
    ...
    [[self getCurrentVC] presentViewController:alert animated:NO completion:^{
        [[self getCurrentVC].rdv_tabBarController setTabBarHidden:YES animated:YES];
        
        alert.WScale_alert = WScale;
        alert.HScale_alert = HScale;
        
        [alert showView];
        alert.titleLabel.text = LocalString(@"警报");
        alert.messageLabel.text = LocalString(@"超温报警！！！");
        [alert.centeredBtn setTitle:LocalString(@"知道了") forState:UIControlStateNormal];
    }];
}
...
- (void)bakeCompeleteWithCurveDataDic:(NSDictionary *)curveDataDic{
    ...
    curveName = [NSString stringWithFormat:@"%@%@",LocalString(@"烘焙报告"),[formatter stringFromDate:date]];
...
//        [set close];
    ...
    if (isSucc) {
        ...
        CurveViewController *curve = (CurveViewController *)[viewControllers firstObject];
        [curve loadViewIfNeeded];
        //新建设置语言页面
        ...
        [viewControllers addObject:reportVC];
        nav.viewControllers = viewControllers;
        _isNeedRegetStatus = YES;
        //[_myTimer setFireDate:[NSDate distantFuture]];
    }
}
...
- (NSMutableArray *)getBeanTempRorWithArr:(NSMutableArray *)arr{
    ...
    //每秒显示数据0～15，1～16。。。依次显示
    for (NSInteger i = beanRorDiffCount; i < [arr count]; i = i + 1) {
...
//    double y;
//    //每秒显示数据0～15，1～16。。。依次显示
//    for (NSInteger i = 1; i < [arr count]; i = i + 1) {
//        y = ([NSString diffTempUnitStringWithTemp:[arr[i] doubleValue]] - [NSString diffTempUnitStringWithTemp:[arr[i - 1] doubleValue]]) * (60.f/beanRorDiffCount);
//
//        [yArr addObject:[NSNumber numberWithDouble:y]];
//
//        [rorArr addObject:[[ChartDataEntry alloc] initWithX:i y:[[self linearSmooth:yArr][i] doubleValue]]];
//    }
//    return rorArr;
//}
//
//#pragma mark - 处理ROR数据平滑
//- (NSMutableArray *)linearSmooth:(NSMutableArray *)ror{
//    NSMutableArray *smoothRor = [[NSMutableArray alloc]init];
//    double total = 0.0,average = 0.0;
//    for (NSInteger i = beanRorDiffCount; i < [ror count]; i = i + 1) {
//        total = (total + [ror[i] doubleValue]);
//        average = total/beanRorDiffCount;
//        [smoothRor addObject:[NSNumber numberWithDouble:average]];
//    }
//    return smoothRor;
//}
