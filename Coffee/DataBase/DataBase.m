...
- (SettingModel *)setting{
    if (!_setting) {
        ...
        _setting.tempUnit = @"℃";
        _setting.bakeChromaReferStandard = @"agron";
        _setting.timeAxis = 15;
        _setting.tempAxis = 350;
        _setting.tempCurveSmooth = 6;//现在已经不用了，先留着
        _setting.tempRateSmooth = 30;
        _setting.language = LocalString(@"简体中文");//这个属性作废了，语言信息存在userDefault里
    }
    return _setting;
}
    ...
    [manager GET:url parameters:nil headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if ([[responseObject objectForKey:@"errno"] intValue] == 0) {
            if ([responseObject objectForKey:@"data"]) {
                NSDictionary *settingDic = [responseObject objectForKey:@"data"];
                if (settingDic == nil) {
