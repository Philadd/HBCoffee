...
- (void)getClientInfoByApi{
    ...
    [manager GET:url parameters:nil headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if ([[responseObject objectForKey:@"statusCode"] intValue] == 200) {
            NSLog(@"success:%@",daetr);
            accessKeySecret = [responseObject objectForKey:@"accessKeySecret"];
            accessKeyId = [responseObject objectForKey:@"accessKeyId"];
            expiration = [responseObject objectForKey:@"expiration"];
            securityToken = [responseObject objectForKey:@"securityToken"];
            [self initOOSClient];
        }else{
            NSLog(@"获取sts信息失败");
        }
