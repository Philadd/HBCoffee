...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            cell.rightLabel.text = [NSString stringWithFormat:@"%ld%@",(long)_myData.setting.timeAxis,LocalString(@"分钟")];
            return cell;
        }else{
        ...
        cell.rightLabel.text = [NSString stringWithFormat:@"%d",_myData.setting.tempRateSmooth/10];
        return cell;
        ...
        if (language == NULL || [language isEqualToString:@""]) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 1) {
        if (indexPath.row == 0) {
            [self showSheetWithTitle:LocalString(@"请选择重量单位") actions:@[@"g",@"kg"] indexpath:indexPath];
        }
        if (indexPath.row == 1) {
    ...
    if (indexPath.section == 2) {
        if (indexPath.row == 0) {
            /*
             时间轴区间8~29，默认15；温度轴区间150~500，默认350
             */
            ...
            VC.pickerBlock = ^(NSInteger picker) {
                self.myData.setting.timeAxis =  [array[picker] integerValue];
                [self.generalTable reloadData];
                [self updateSetting];
            };
            ...
            VC.pickerBlock = ^(NSInteger picker) {
                self.myData.setting.tempAxis =  [array[picker] integerValue];
                [self.generalTable reloadData];
                [self updateSetting];
            };
            VC.modalPresentationStyle = UIModalPresentationOverCurrentContext;
    ...
    if (indexPath.section == 3) {
        if (indexPath.row == 0) {
            ...
            VC.pickerArr = [array mutableCopy];
            VC.pickerBlock = ^(NSInteger picker) {
                self.myData.setting.tempRateSmooth = [array[picker] integerValue]*10;
                [self.generalTable reloadData];
                [self updateSetting];
            };
            VC.modalPresentationStyle = UIModalPresentationOverCurrentContext;
        ...
        UIAlertAction *alertAction = [UIAlertAction actionWithTitle:actionTitle style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (indexpath.section == 1) {
                    ...
                    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
                    [userDefaults setObject:actionTitle forKey:@"language"];
                    [userDefaults synchronize];
