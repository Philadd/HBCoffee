...
- (void)loginAction{
    if ([self.numPassword intValue] > 4 ) {
...
- (void)login:(NSString *)mobile password:(NSString *)pw{
    ...
    NSData * data = [NSJSONSerialization dataWithJSONObject:headers options:(NSJSONWritingOptions)0 error:nil];
    NSDictionary *responseObject = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers|NSJSONReadingMutableLeaves error:nil];
    
    [manager POST:@"http://139.196.90.97:8080/coffee/user/login" parameters:parameters headers:responseObject progress:nil
          success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
              //保存账号密码
              NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
              BOOL isECSUpdate = [[userDefaults objectForKey:self.phoneTF.text] boolValue];//判断上一次登陆时是否获取了全部信息，还是因为网络问题中断了，需要再次请求所有信息获取更新
              ...
              [userDefaults synchronize];
              //AFHTTPSessionManager 内部已经json解析 responseObject为字典
              ...
              if ([[responseObject objectForKey:@"errno"] intValue] == 0) {
                  ...
                  if ([[responseObject objectForKey:@"error"] isEqualToString:@"lock"]) {
                      ...
                      //防止手机错误
                      if ([[responseObject objectForKey:@"error"] isEqualToString:@"手机号或者验证码错误"]) {
                          [NSObject showHudTipStr:LocalString(@"手机号或者验证码错误")];
                          return;
                      }
                      self.numPassword = [responseObject objectForKey:@"error"];
                      NSString *megStr = [NSString stringWithFormat:@"%@ %@",LocalString(@"手机号或者密码错误，剩余登录次数为"),self.numPassword];
                      [NSObject showHudTipStr:megStr];
