...
//    return;
    if (_myNet.connectedDevice) {
        //防止数据未加载
        if ([_environTempLabel.text hasPrefix:@"0.0"]) {
            [NSObject showHudTipStr:LocalString(@"请等待温度初始化")];
            return;
        }
        BakeCurveViewController *bakeCurveVC = [[BakeCurveViewController alloc] init];
        bakeCurveVC.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
...
//            _mainView.frame = CGRectMake(0, (ScreenHeight - 64 - (tabbarHeight + kSafeArea_Bottom) - 400/HScale - 90/HScale), size.width, size.height);//154是控制按钮view高度，刚好把控制按钮隐藏，90是bottonview高度
            self.mainView.frame = CGRectMake(0, 37.f/HScale, size.width, size.height);//154是控制按钮view高度，刚好把控制按钮隐藏，90是bottonview高度
        ...
        [UIView animateWithDuration:0.2 animations:^{
            CGSize size = self.mainView.frame.size;
            self.mainView.frame = CGRectMake(0, (ScreenHeight - 64 - (tabbarHeight + kSafeArea_Bottom) - 400/HScale + 154/HScale - 90/HScale), size.width, size.height);//154是控制按钮view高度，刚好把控制按钮隐藏，90是bottonview高度
...
- (void)showPowerConfirmAlert{
    if (!_myNet.connectedDevice || [self.environTempLabel.text hasPrefix:@"0.0"]) {
...
- (void)getCoffeeWorkImage{
    //连接上设备之后刷新设备图片
    switch ([self.myNet.connectedDevice.deviceType integerValue]) {
        case Coffee_HB_M6G:
        {
            self.deviceImage.frame = CGRectMake(75/WScale, 18/HScale, 225/WScale, 150/HScale);
            self.deviceImage.image = [UIImage imageNamed:@"img_hb_m6g_small"];
        }
            break;
        case Coffee_HB_L2:
        {
            self.deviceImage.frame = CGRectMake(75/WScale, 18/HScale, 225/WScale, 150/HScale);
            self.deviceImage.image = [UIImage imageNamed:@"img_hb_l2_small"];
        }
            break;
        case Coffee_PEAK_Edmund:
        {
            self.deviceImage.frame = CGRectMake(75/WScale, 18/HScale, 225/WScale, 150/HScale);
            self.deviceImage.image = [UIImage imageNamed:@"img_peak_edmund"];
        }
            break;
...
- (void)deviceImageHandleTapGesture:(UITapGestureRecognizer *)tapRecognizer{
    ...
    if (_showControlViewBtn.tag == unselect) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            CGSize size = self.mainView.frame.size;
//            _mainView.frame = CGRectMake(0, (ScreenHeight - 64 - (tabbarHeight + kSafeArea_Bottom) - 400/HScale - 90/HScale), size.width, size.height);//154是控制按钮view高度，刚好把控制按钮隐藏，90是bottonview高度
            self.mainView.frame = CGRectMake(0, 37.f, size.width, size.height);//154是控制按钮view高度，刚好把控制按钮隐藏，90是bottonview高度
            [self.showControlViewBtn setImage:[UIImage imageNamed:@"btn_collapse"] forState:UIControlStateNormal];
        }];
    }else{
        ...
        [UIView animateWithDuration:0.2 animations:^{
            CGSize size = self.mainView.frame.size;
            self.mainView.frame = CGRectMake(0, (ScreenHeight - 64 - (tabbarHeight + kSafeArea_Bottom) - 400/HScale + 154/HScale - 90/HScale), size.width, size.height);//154是控制按钮view高度，刚好把控制按钮隐藏，90是bottonview高度
            [self.showControlViewBtn setImage:[UIImage imageNamed:@"btn_expand"] forState:UIControlStateNormal];
        }];
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context{
    ...
    if ([keyPath isEqualToString:@"tempData"]) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            //当顺时针旋转120度时，指针转到129
            self.pointerImage.transform = CGAffineTransformMakeRotation((tempBean / 129.f * 120.0) / 180 * M_PI);
            if (tempBean > net.alertTemp && net.alertTemp > 0.f) {
                self.beanTempLabel.textColor = [UIColor colorWithHexString:@"FF4733"];
                NSLog(@"f风飒风%f %f",net.alertTemp,tempBean);
            }else{
                self.beanTempLabel.textColor = [UIColor colorWithHexString:@"4778CC"];
            }
            self.beanTempLabel.text = [NSString stringWithFormat:@"%.1f",[NSString diffTempUnitStringWithTemp:tempBean]];
            self.inTempLabel.text = [NSString stringWithFormat:@"%.1f%@",[NSString diffTempUnitStringWithTemp:tempIn],[DataBase shareDataBase].setting.tempUnit];
            self.outTempLabel.text = [NSString stringWithFormat:@"%.1f%@",[NSString diffTempUnitStringWithTemp:tempOut],[DataBase shareDataBase].setting.tempUnit];
            self.environTempLabel.text = [NSString stringWithFormat:@"%.1f%@",[NSString diffTempUnitStringWithTemp:tempEnvironment],[DataBase shareDataBase].setting.tempUnit];
            if (net.BeanArr.count > 5) {
                self.beanTempRateLabel.text = [NSString stringWithFormat:@"%.1f%@/min",([NSString diffTempUnitStringWithTemp:[net.BeanArr[net.BeanArr.count-1] doubleValue]] - [NSString diffTempUnitStringWithTemp:[net.BeanArr[net.BeanArr.count-6] doubleValue]])/5*60,[DataBase shareDataBase].setting.tempUnit];
            }
        });
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.myNet.powerStatus) {
                [self.powerBtn setImage:[UIImage imageWithContentsOfFile:[self.resourcePath stringByAppendingPathComponent:@"btn_power_on@2x.png"]] forState:UIControlStateNormal];
                self.fireBtn.enabled = YES;
                self.coldBtn.enabled = YES;
                self.stirBtn.enabled = YES;
                self.windPBtn.enabled = NO;
                self.firePBtn.enabled = NO;
                self.powerBtn.enabled = YES;
                self.status.text = LocalString(@"电源开启");
                self.statusView.layer.backgroundColor = [UIColor colorWithRed:126/255.0 green:211/255.0 blue:33/255.0 alpha:1].CGColor;
                self.statusView.layer.shadowColor = [UIColor colorWithRed:106/255.0 green:255/255.0 blue:77/255.0 alpha:1].CGColor;
                self.statusView.layer.shadowOffset = CGSizeMake(0,0);
                self.statusView.layer.shadowOpacity = 1;
                self.statusView.layer.shadowRadius = 8;
                
                if ([net.connectedDevice.deviceType integerValue] != Coffee_HB_Another) {
                    self.deviceWorkImage.hidden = NO;
                }
            }else{
                //电源关闭后所有按钮关闭
                [self.powerBtn setImage:[UIImage imageWithContentsOfFile:[self.resourcePath stringByAppendingPathComponent:@"btn_power_off@2x.png"]] forState:UIControlStateNormal];
                [self.fireBtn setImage:[UIImage imageWithContentsOfFile:[self.resourcePath stringByAppendingPathComponent:@"btn_fire_off@2x.png"]] forState:UIControlStateNormal];
                [self.coldBtn setImage:[UIImage imageWithContentsOfFile:[self.resourcePath stringByAppendingPathComponent:@"btn_cold_off@2x.png"]] forState:UIControlStateNormal];
                [self.stirBtn setImage:[UIImage imageWithContentsOfFile:[self.resourcePath stringByAppendingPathComponent:@"btn_stir_off@2x.png"]] forState:UIControlStateNormal];
                
                //所有状态为关闭
                self.myNet.coolStatus = NO;
                self.myNet.fireStatus = NO;
                self.myNet.stirStatus = NO;
                
                //所有功能不能使用
                self.fireBtn.enabled = NO;
                self.coldBtn.enabled = NO;
                self.stirBtn.enabled = NO;
                self.windPBtn.enabled = NO;
                self.firePBtn.enabled = NO;
                self.powerBtn.enabled = YES;
                self.status.text = LocalString(@"电源关闭");
                self.statusView.layer.backgroundColor = [UIColor colorWithRed:254/255.0 green:71/255.0 blue:51/255.0 alpha:1].CGColor;
                self.statusView.layer.shadowColor = [UIColor colorWithRed:254/255.0 green:71/255.0 blue:52/255.0 alpha:1].CGColor;
                self.statusView.layer.shadowOffset = CGSizeMake(0,0);
                self.statusView.layer.shadowOpacity = 1;
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.myNet.fireStatus) {
                [self.fireBtn setImage:[UIImage imageWithContentsOfFile:[self.resourcePath stringByAppendingPathComponent:@"btn_fire_on@2x.png"]] forState:UIControlStateNormal];
                if ([net.connectedDevice.deviceType integerValue] != Coffee_HB_Another) {
                    self.rejiImage.hidden = NO;
                }
                
                self.status1.textColor = [UIColor colorWithHexString:@"333333"];
                
                self.statusView1.layer.backgroundColor = [UIColor colorWithRed:255/255.0 green:128/255.0 blue:0/255.0 alpha:1].CGColor;
                self.statusView1.layer.shadowColor = [UIColor colorWithRed:255/255.0 green:128/255.0 blue:0/255.0 alpha:1].CGColor;
                self.statusView1.layer.shadowOffset = CGSizeMake(0,0);
                self.statusView1.layer.shadowOpacity = 1;
                self.statusView1.layer.shadowRadius = 8;
            }else{
                [self.fireBtn setImage:[UIImage imageWithContentsOfFile:[self.resourcePath stringByAppendingPathComponent:@"btn_fire_off@2x.png"]] forState:UIControlStateNormal];
                self.rejiImage.hidden = YES;
                
                self.status1.textColor = [UIColor colorWithHexString:@"999999"];
                
                self.statusView1.layer.backgroundColor = [UIColor colorWithRed:213/255.0 green:218/255.0 blue:224/255.0 alpha:1].CGColor;
                self.statusView1.layer.shadowOpacity = 0;
            }
        });
    }else if ([keyPath isEqualToString:@"coolStatus"]){
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.myNet.coolStatus) {
                [self.coldBtn setImage:[UIImage imageWithContentsOfFile:[self.resourcePath stringByAppendingPathComponent:@"btn_cold_on@2x.png"]] forState:UIControlStateNormal];
                if ([net.connectedDevice.deviceType integerValue] != Coffee_HB_Another) {
                    self.lengqueImage.hidden = NO;
                }
                
                self.status3.textColor = [UIColor colorWithHexString:@"333333"];
                
                self.statusView3.layer.backgroundColor = [UIColor colorWithRed:73/255.0 green:152/255.0 blue:242/255.0 alpha:1].CGColor;
                self.statusView3.layer.shadowColor = [UIColor colorWithRed:73/255.0 green:152/255.0 blue:242/255.0 alpha:1].CGColor;
                self.statusView3.layer.shadowOffset = CGSizeMake(0,0);
                self.statusView3.layer.shadowOpacity = 1;
                self.statusView3.layer.shadowRadius = 8;
            }else{
                [self.coldBtn setImage:[UIImage imageWithContentsOfFile:[self.resourcePath stringByAppendingPathComponent:@"btn_cold_off@2x.png"]] forState:UIControlStateNormal];
                self.lengqueImage.hidden = YES;
                
                self.status3.textColor = [UIColor colorWithHexString:@"999999"];
                
                self.statusView3.layer.backgroundColor = [UIColor colorWithRed:213/255.0 green:218/255.0 blue:224/255.0 alpha:1].CGColor;
                self.statusView3.layer.shadowOpacity = 0;
            }
        });
    }else if ([keyPath isEqualToString:@"stirStatus"]){
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.myNet.stirStatus) {
                [self.stirBtn setImage:[UIImage imageWithContentsOfFile:[self.resourcePath stringByAppendingPathComponent:@"btn_stir_on@2x.png"]] forState:UIControlStateNormal];
            }else{
                [self.stirBtn setImage:[UIImage imageWithContentsOfFile:[self.resourcePath stringByAppendingPathComponent:@"btn_stir_off@2x.png"]] forState:UIControlStateNormal];
            }
        });
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.myNet.connectedDevice && ![self.environTempLabel.text hasPrefix:@"0.0"]) {
                self.status.text = LocalString(@"设备已连接");
                self.statusView.layer.backgroundColor = [UIColor colorWithRed:126/255.0 green:211/255.0 blue:33/255.0 alpha:1].CGColor;
                self.statusView.layer.shadowColor = [UIColor colorWithRed:106/255.0 green:255/255.0 blue:77/255.0 alpha:1].CGColor;
                self.statusView.layer.shadowOffset = CGSizeMake(0,0);
                self.statusView.layer.shadowOpacity = 1;
                ...
                self.status.text = LocalString(@"设备未连接");
                self.statusView.layer.backgroundColor = [UIColor colorWithRed:254/255.0 green:71/255.0 blue:51/255.0 alpha:1].CGColor;
                self.statusView.layer.shadowColor = [UIColor colorWithRed:254/255.0 green:71/255.0 blue:52/255.0 alpha:1].CGColor;
                self.statusView.layer.shadowOffset = CGSizeMake(0,0);
                self.statusView.layer.shadowOpacity = 1;
                
                self.beanTempLabel.text = [NSString stringWithFormat:@"%.1f",0.0];
                self.inTempLabel.text = [NSString stringWithFormat:@"%.1f%@",0.0,[DataBase shareDataBase].setting.tempUnit];
                self.outTempLabel.text = [NSString stringWithFormat:@"%.1f%@",0.0,[DataBase shareDataBase].setting.tempUnit];
                self.environTempLabel.text = [NSString stringWithFormat:@"%.1f%@",0.0,[DataBase shareDataBase].setting.tempUnit];
                self.beanTempRateLabel.text = [NSString stringWithFormat:@"%.1f%@/min",0.0,[DataBase shareDataBase].setting.tempUnit];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (!self.myNet.deviceTimerStatus && self.myNet.connectedDevice) {
                self.statusView2.layer.backgroundColor = [UIColor colorWithRed:255/255.0 green:221/255.0 blue:51/255.0 alpha:1.0].CGColor;
                self.hongbeiImage.hidden = NO;
            }else{
                self.statusView2.layer.backgroundColor = [UIColor colorWithRed:213/255.0 green:218/255.0 blue:224/255.0 alpha:1].CGColor;
                self.hongbeiImage.hidden = YES;;
            }
        });
    }
...
- (UIView *)mainView{
    if (!_mainView) {
        ...
        [_powerBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.centerX.equalTo(self.mainView.mas_centerX).offset(-125/WScale);
            make.top.equalTo(self.mainView.mas_top).offset(246/HScale);
        }];
        ...
        [_coldBtn mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(50/WScale, 50/WScale));
            make.centerX.equalTo(self.mainView.mas_centerX).offset(-125/WScale);
            make.top.equalTo(self.mainView.mas_top).offset(326/HScale);
        }];
        ...
        [powerLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.centerX.equalTo(self.powerBtn.mas_centerX);
            make.top.equalTo(self.powerBtn.mas_bottom).offset(2/HScale);
        }];
        [fireLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(80/WScale, 19/HScale));
            make.centerX.equalTo(self.fireBtn.mas_centerX);
            make.top.equalTo(self.fireBtn.mas_bottom).offset(2/HScale);
        }];
        [stirLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(80/WScale, 19/HScale));
            make.centerX.equalTo(self.stirBtn.mas_centerX);
            make.top.equalTo(self.stirBtn.mas_bottom).offset(2/HScale);
        }];
        [coldLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(80/WScale, 19/HScale));
            make.centerX.equalTo(self.coldBtn.mas_centerX);
            make.top.equalTo(self.coldBtn.mas_bottom).offset(2/HScale);
        }];
        [firepLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(80/WScale, 19/HScale));
            make.centerX.equalTo(self.firePBtn.mas_centerX);
            make.top.equalTo(self.firePBtn.mas_bottom).offset(2/HScale);
        }];
        [windpLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(80/WScale, 19/HScale));
            make.centerX.equalTo(self.windPBtn.mas_centerX);
            make.top.equalTo(self.windPBtn.mas_bottom).offset(2/HScale);
        }];
    }
    return _mainView;
}
...
- (UILabel *)beanTempLabel{
    if (!_beanTempLabel) {
        ...
        [textLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(self.beanTempLabel.mas_right).offset(4/WScale);
            make.top.equalTo(self.mainView.mas_top).offset(47/HScale);
        }];
        ...
        [_beanTempUnitLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(self.beanTempLabel.mas_right).offset(4/WScale);
            make.top.equalTo(self.mainView.mas_top).offset(65/HScale);
        }];
    }
...
- (UILabel *)inTempLabel{
    if (!_inTempLabel) {
        ...
        [textLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(self.inTempLabel.mas_left);
            make.top.equalTo(self.inTempLabel.mas_bottom);
        }];
        ...
        [image mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.right.equalTo(self.inTempLabel.mas_left);
            make.top.equalTo(self.inTempLabel.mas_top);
        }];
    }
    return _inTempLabel;
}
...
- (UILabel *)outTempLabel{
    if (!_outTempLabel) {
        ...
        [textLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(50/WScale, 17/HScale));
            make.left.equalTo(self.outTempLabel.mas_left);
            make.top.equalTo(self.outTempLabel.mas_bottom);
        }];
        ...
        [image mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(40/WScale, 40/WScale));
            make.right.equalTo(self.outTempLabel.mas_left);
            make.top.equalTo(self.outTempLabel.mas_top);
        }];
    }
...
- (UILabel *)environTempLabel{
    if (!_environTempLabel) {
        ...
        [textLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.equalTo(self.environTempLabel.mas_left);
            make.top.equalTo(self.environTempLabel.mas_bottom);
        }];
        ...
        [image mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(40/WScale, 40/WScale));
            make.right.equalTo(self.environTempLabel.mas_left);
            make.top.equalTo(self.environTempLabel.mas_top);
        }];
    }
...
- (UIScrollView *)beanNameView{
    ...
    [_beanNameView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self.mainView.mas_left).offset(205/WScale);
        make.top.equalTo(self.beanTempRateLabel.mas_bottom).offset(18/HScale);
    }];
