...
- (UIView *)noCurveView{
    if (!_noCurveView) {
        ...
        [deviceImage mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.centerX.equalTo(self.noCurveView.mas_centerX);
            make.top.equalTo(self.noCurveView.mas_top).offset(120.f / HScale);
        }];
        
        [_noCurveLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.size.mas_equalTo(CGSizeMake(ScreenWidth, 20.f / HScale));
            make.centerX.equalTo(self.noCurveView.mas_centerX);
            make.top.equalTo(self.noCurveView.mas_top).offset(252.f / HScale);
        }];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSLog(@"ssss%ld",[_currentReportArr[0] count]);
    ReportModel *report = _currentReportArr[indexPath.section][indexPath.row];
    if (report.isShare == 0) {
    ...
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [manager DELETE:url parameters:nil headers:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            ...
            if ([[responseObject objectForKey:@"errno"] intValue] == 0) {
                ...
                if (result) {
                    if ([self.currentReportArr[indexPath.section] count] == 0) {
                        [self.currentReportArr removeObjectAtIndex:indexPath.section];
                        [tableView deleteSections:[NSIndexSet indexSetWithIndex:indexPath.section] withRowAnimation:UITableViewRowAnimationLeft];
                    }else{
                        [self.currentReportArr[indexPath.section] removeObjectAtIndex:indexPath.row];
...
-(void)reloadIndexReport{
    
    NSInteger Index = _mySegment.selectedSegmentIndex;
    switch (Index) {
        case 0:
        {
            [_currentReportArr removeAllObjects];
