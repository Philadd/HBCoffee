...
- (void)setNavItem{
    ...
    [leftButton addTarget:self action:@selector(deleteDevice) forControlEvents:UIControlEventTouchUpInside];
    ...
    [rightButton addTarget:self action:@selector(saveEdit) forControlEvents:UIControlEventTouchUpInside];
...
- (void)deleteDevice{
    ...
    //本地数据库delete
    BOOL result = NO;
    result = [db deleteqReport:self.reportModel];
    //返回根视图
    [self.navigationController popToRootViewControllerAnimated:YES];
    
    [self deleteCurveByApi];
}
...
- (void)deleteCurveByApi{
    ...
    NSString *url = [NSString stringWithFormat:@"http://139.196.90.97:8080/coffee/roastCurve/deleteReport?curveUid=%@&num=%ld",_reportModel.curveUid,(long)_reportModel.isShare];
    url = [url stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet characterSetWithCharactersInString:@"`#%^{}\"[]|\\<> "].invertedSet];
