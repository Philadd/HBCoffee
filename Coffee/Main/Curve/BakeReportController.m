...
NSString *const CellIdentifier_reportCurveDetail = @"CellID_reportCurveDetail";
...
- (UITableView *)reportTable{
    if (!_reportTable) {
        _reportTable = ({
            ...
            [tableView registerClass:[CurveDetailClickCell class] forCellReuseIdentifier:CellIdentifier_reportCurveDetail];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        ...
        NSLog(@"aaaaa%d",(int)_reportModel.light);
        ...
        if (indexPath.row == 0) {
            ...
            CurveDetailClickCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier_reportCurveDetail];
            if (cell == nil) {
                cell = [[CurveDetailClickCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier_reportCurveDetail];
            }
            return cell;
        }
...
- (void)queryReportInfo{
    ...
    if (_reportModel.curveValueJson) {
        ...
        NSLog(@"ffff %lu%lu%lu%lu %ld",(unsigned long)_Bean.count,_Out.count,_In.count,_Environment.count,(long)beanRorDiffCount);
        if (!_Diff) {
            _Diff = [[NSMutableArray alloc] init];
        }
        [_Diff removeAllObjects];
        for (long i = beanRorDiffCount; i < _Bean.count; i = i + 1) {
...
- (void)queryEventInfo{
    ...
    for (EventModel *event in _eventArray) {
        ...
        NSInteger count = _yVals_Bean.count;
        if (event.eventTime >= count) {//超出时减去补偿
            ChartDataEntry *entry = _yVals_Bean[count - 1];
