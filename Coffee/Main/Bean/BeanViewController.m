...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath{
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        NSMutableArray *beanArray = [[DataBase shareDataBase] queryAllBean];
        if (beanArray.count <= 1) {
            [NSObject showHudTipStr:LocalString(@"删除失败,必须要保留一种豆.")];
            return;
        }
        ...
        NSString *url = [NSString stringWithFormat:@"http://139.196.90.97:8080/coffee/bean?beanUid=%@",bean.beanUid];
